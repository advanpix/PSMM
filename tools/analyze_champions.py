...
def main():
    ...
    with OUT.open("w") as out:
        ...
        print("", file=out)
        print("This is consistent with the **Salem–Boyd theorem** that limits", file=out)
        print("of cyclotomic-perturbed polynomial families are themselves Salem", file=out)
        print("numbers, not transcendental limits. Generating sub-Lehmer", file=out)
        print("polynomials (if any exist) would require a fundamentally", file=out)
        print("different perturbation pattern — varying $\\Phi_d$ for $d > 3$,", file=out)
        print("using larger perturbations, or composing several perturbations.", file=out)
        print("", file=out)
        print("### Suggested next experiments", file=out)
        print("", file=out)
        print("- **Vary the perturbation cyclotomic**: scan $\\Phi_d$ for", file=out)
        print("  $d \\in \\{4, 5, 6, 7, 8, 12, \\ldots\\}$ in place of $\\Phi_3$.", file=out)
        print("- **Vary the perturbation position**: instead of placing $\\Phi_3$", file=out)
        print("  at the exact half-degree, scan nearby positions.", file=out)
        print("- **Multiple perturbations**: subtract $x^{a}\\Phi_3 + x^{b}\\Phi_5$", file=out)
        print("  and check if any $(m, a, b)$ produces a smaller-than-Lehmer", file=out)
        print("  irreducible factor.", file=out)
        ...
        print("", file=out)
        print("Each of these is a directed search with a small parameter space,", file=out)
        print("*much* smaller than the brute-force PSMM enumeration over all", file=out)
        print("integer coefficient sparse polynomials. The cyclotomic-perturbation", file=out)
        print("structural insight reduces the search space from $\\sim 10^{12}$", file=out)
        print("polynomials per degree to $\\sim 10^2$.", file=out)
        print("", file=out)
