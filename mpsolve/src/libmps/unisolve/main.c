 ...
 */
#if defined(_MSC_VER)
#   define MPSOLVE_MAHLER_THREAD_ATTR __declspec( thread )
#else
#   define MPSOLVE_MAHLER_THREAD_ATTR __thread
#endif

static MPSOLVE_MAHLER_THREAD_ATTR int glb_over_threshold = 0;
static MPSOLVE_MAHLER_THREAD_ATTR double glb_threshold = -1.0;

void mps_mahler_mpsolve_implementation (mps_context * s);
mps_boolean mps_mahler_check_stop (mps_context * s);

void mps_mahler_set_threshold(double threshold)
{
   glb_threshold = threshold;
}
...
void mps_mahler_mpsolve (mps_context * s)
{
  ...
  if (!s->active_poly->thread_safe)
    mps_thread_pool_set_concurrency_limit (s, NULL, 1);

  if(glb_threshold > 0.0)
    glb_over_threshold = 0;

  mps_mahler_mpsolve_implementation(s);
}

void
mps_mahler_mpsolve_implementation (mps_context * s)
{
  int i, nzc;
  char which_case;
  mps_boolean d_after_f, computed;
